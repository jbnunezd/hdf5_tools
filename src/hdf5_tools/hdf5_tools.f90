...
IF (STATUS .EQ. "OLD") THEN
  ...
  ELSE
    ErrorMessage = "HDF5_OpenFile: ACTION = "//TRIM(ACTION)//" not supported."
    CALL PrintError(__STAMP__,ErrorMessage)
  END IF
ELSEIF (STATUS .EQ. "NEW") THEN
  IF (ACTION .EQ. "READ") THEN
    ErrorMessage = "HDF5_OpenFile: STATUS = "//TRIM(STATUS)//" combinated with ACTION = "// &
      TRIM(ACTION)//" not supported."
    CALL PrintError(__STAMP__,ErrorMessage)
  ELSEIF ((ACTION .EQ. "WRITE") .OR. (ACTION .EQ. "READWRITE")) THEN
...
ELSE
  ErrorMessage = "HDF5_OpenFile: STATUS = "//TRIM(STATUS)//" not supported."
  CALL PrintError(__STAMP__,ErrorMessage)
END IF
...
ELSE
  ErrorMessage = "HDF5_CreateDataSet: DSET_TYPE = "//TRIM(dset_type)//" not supported."
  CALL PrintError(__STAMP__,ErrorMessage)
END IF
